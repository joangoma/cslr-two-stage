...
import math
from transformers import CLIPVisionModel  # Specialized for vision-only tasks
...
class VideoToGlossModel(nn.Module):
    def __init__(
        ...
        # Using CLIPVisionModel directly isolates the image encoder
        self.backbone = CLIPVisionModel.from_pretrained(clip_model_name)
        '''
        for param in self.backbone.parameters():
            param.requires_grad = False

        # Only the last N transformer blocks + the head are trainable initially
        num_unfreeze = 4 
        ...
        # Always unfreeze layer norm and post-layernorm
        ...
        visual_feat_dim = self.backbone.config.hidden_size  # 768 for clip-vit-base-patch32

        # CNN Modification: first layer jumps to 2048 channels, second layer scales back down
        ...
        # Give <blank> (index 0) a higher prior at initialisation.
    ...
    def forward(
        ...
        backbone_out = self.backbone(flat_frames)

        # last_hidden_state[:, 0, :] extracts the [CLS] embedding equivalent from CLIP's vision tower
        feats = backbone_out.last_hidden_state[:, 0, :]
